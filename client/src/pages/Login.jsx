...
const metrics = [
  ...
  { label: "People", value: "Synced", icon: Users }
];

const previewRows = [
  ["Production", "Present", "91%"],
  ["HR Desk", "Requests", "04"],
  ["Reports", "Ready", "PDF"]
];
      ...
      <div className="grid min-h-mobile lg:grid-cols-[minmax(0,1fr)_520px]">
        <section className="mobile-safe-top relative flex min-h-[46vh] flex-col justify-between overflow-hidden bg-[#101820] px-5 py-6 text-white sm:px-6 sm:py-8 lg:min-h-screen lg:px-12 lg:py-10">
          ...
          <div className="relative">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-base font-black text-white">HYA Tech</p>
                  <p className="text-xs font-bold text-slate-300">Employee Management</p>
                </div>
              </div>
              <span className="hidden rounded-full bg-white/10 px-3 py-1.5 text-xs font-black uppercase tracking-[0.18em] text-emerald-100 ring-1 ring-white/15 sm:inline-flex">
                Secure HRMS
              </span>
            </div>
          </div>
          ...
          <div className="relative my-10 max-w-3xl lg:my-0">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1.5 text-xs font-bold text-blue-50 ring-1 ring-white/15">
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              {metrics.map((item) => {
                ...
                return (
                  <div className="rounded-lg bg-white/10 p-4 ring-1 ring-white/15" key={item.label}>
                    <Icon className="h-5 w-5 text-emerald-200" aria-hidden="true" />
                    <p className="mt-3 text-xs font-bold uppercase tracking-wide text-slate-300">{item.label}</p>
            ...
            <div className="mt-8 max-w-2xl rounded-lg border border-white/15 bg-white/[0.08] p-4 shadow-2xl shadow-black/20 backdrop-blur">
              <div className="mb-4 flex items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-black text-white">Today Command Center</p>
                  <p className="text-xs font-semibold text-slate-300">Live operating snapshot</p>
                </div>
                <span className="inline-flex items-center gap-1 rounded-full bg-emerald-400/15 px-2.5 py-1 text-xs font-black text-emerald-100 ring-1 ring-emerald-300/20">
              ...
              <div className="space-y-2">
                {previewRows.map(([area, status, value]) => (
                  <div className="grid grid-cols-[1fr_auto_auto] items-center gap-3 rounded-lg bg-white/10 px-3 py-3 ring-1 ring-white/10" key={area}>
                    <span className="truncate text-sm font-bold text-white">{area}</span>
                    <span className="rounded-full bg-white/10 px-2 py-1 text-xs font-bold text-slate-200">{status}</span>
                    <span className="text-sm font-black text-emerald-100">{value}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="relative flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-300">
            ...
            <span>Built for HYA Tech teams</span>
          </div>
        </section>
        ...
        <section className="mobile-safe-page flex items-center justify-center px-5 py-8 sm:px-6 lg:px-12">
          ...
          >
            ...
            <div className="mt-4 flex items-center justify-between gap-3">
              <Link className="text-sm font-semibold text-hya-700 hover:text-hya-900" to="/forgot-password">
                Forgot password
              </Link>
            </div>
